...
`/memory verify <id>`（全量复验该实体）触发。追加任何记录前，先读本手册、entities.md 与 verifications.md。
...
- 追加记录前先定级（判据与定级纪律见 verifications.md）：**等级记高了比不记更糟**——它让下一次读者跳过本该做的复核。
...
3. 事实已无价值：正文保持不动，追加 failed 记录说明废弃理由。实体留在库中（failed 进修正流，
   若正文与最新 failed 记录均未变，`verify all` 会重复同一结论；需彻底停止重验请删除实体文件——**出库只有这一条路，由人执行**（扩展永不删）。
