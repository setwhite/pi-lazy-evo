...
- 协议（protocol/）：schema.md 共享规则 + record/query/verify 三份操作手册——agent 执行契约；
...
```
lazy-memory/                    ← 单一项目仓库（v1 历史保留在 git，实体已清）
├── README.md                   项目说明（对外英文）
├── LICENSE                     MIT（沿用）
...
│   ├── protocol/             协议：schema.md 共享规则 + record/query/verify 手册
...
| protocol | 协议文档 | schema.md 共享规则单点维护 + record/query/verify 手册，agent 执行契约 |
...
- [x] 协议拆分去重：schema.md 共享规则单点维护 + 三份操作手册；仓库根 v1 skill 目录实体删除

待办（需确认后执行）：

- [ ] 机外副本清理：删除 `AGI/.pi/skills/lazy-memory/` 等 v1 安装副本（仓库内已清；
      v2 未安装前删之则 pi 暂时无 lazy-memory，需用户确认）
...
- 协议目录（protocol/）由 Runtime 注入（`index.ts` 默认与源码同目录），
  提示词层不感知部署位置；protocol/ 移动时改 `index.ts` 一处即可。
